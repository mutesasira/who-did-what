...
export function useLoader() {
  ...
  return useQuery<any, Error>("initial", async () => {
    const { dataSets: { dataSets }, programs: { programs } }: any = await engine.query(query);
...
export function useEnrollmentCount(page = 1, pageSize = 10, query = "", startDate = "", endDate = "") {
  const engine = useDataEngine();

  let params: any = {
    fields: 'phoneNumber,displayName,userCredentials[username,created,lastLogin,createdBy[displayName],lastUpdatedBy[displayName]]',
    page,
    pageSize
  }
  if (query) {
  ...
  return useQuery<any, Error>(["users", page, pageSize, query, startDate, endDate], async () => {
    ...
    const { kIEqe77I6oC: { listGrid: { rows: enrollments } }, kCt1rIMGkJb: { listGrid: { rows: events } } }: any = await engine.query(sqlViewQuery);
...
export function useTEA(program: string, attribute: string, value: string) {
  ...
  return useQuery<any, Error>(["instances", program, attribute, value], async () => {
    const { instances: { trackedEntityInstances } }: any = await engine.query(query);
...
export function useTEI(program: string, value: string) {
  ...
  return useQuery<any, Error>(["instances", program, value], async () => {
    try {
